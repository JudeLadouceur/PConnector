    ...
    public static SaveManager Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                if (_instance == null)
                {
                    GameObject go = new GameObject(typeof(SaveManager).ToString());
    ...
    public void LoadSaveData(int profileNumber = 0)
    {
        ...
        else
        {
            ...
            if (File.Exists(GetSaveDataFilePath(profileNumber)))
            {
                ...
                TimeManager.dayNumber = SaveData.day;
                for (int i = 0; i < SaveData.flagKeys.Count; i++)
                {
                    VariableManager.instance.flags[SaveData.flagKeys[i]] = SaveData.flagValues[i];
                }
                SceneManager.instance.GoToScene(SaveData.sceneNodeID);
            }
            else
            {
